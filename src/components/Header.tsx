...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/80 border-b border-gray-200/40 backdrop-blur-md">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex space-x-8">
            {menuItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="text-[#171513] hover:text-[#C6A537] px-3 py-2 text-sm font-medium tracking-wide transition-colors"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="flex md:hidden">
            <button
              ...
              className="text-[#171513] hover:text-[#C6A537] focus:outline-none transition-colors"
            >
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-white/95 border-b border-gray-200 backdrop-blur-md animate-fade-in">
          <div className="px-2 pt-2 pb-4 space-y-1 sm:px-3">
            {menuItems.map((item) => (
              <Link
                ...
                className="text-[#171513] hover:text-[#C6A537] block px-3 py-3 rounded-md text-base font-medium tracking-wide transition-colors"
              >
                {item.label}
              </Link>
            ))}

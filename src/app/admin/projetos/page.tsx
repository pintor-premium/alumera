...
import { Search, FolderKanban, SlidersHorizontal, MapPin } from 'lucide-react'
...
export default function AdminProjetosPage() {
  ...
  const filteredProjetos = projetos.filter((p) => {
    const matchesSearch = p.nome.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          p.usuarios?.nome_completo.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6 text-[#171513]">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-serif text-2xl font-medium tracking-wide">Todos os Projetos</h1>
          <p className="text-xs text-[#6F6A64] mt-1 font-light">Gerencie e analise todos os projetos técnicos enviados pelos parceiros.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 bg-white p-4 border border-[#E8E5E0] rounded-sm shadow-sm">
        <div className="relative w-full sm:w-80">
          <input
            type="text"
            placeholder="Pesquisar por nome, profissional..."
            value={searchTerm}
      ...
      <div className="bg-white border border-[#E8E5E0] rounded-sm shadow-sm overflow-hidden">
        {filteredProjetos.length === 0 ? (
          <div className="p-16 text-center text-[#6F6A64] space-y-4">
            ...
            <p className="text-sm font-light">Nenhum projeto registrado.</p>
          </div>
        ) : (
                ...
                {filteredProjetos.map((p) => (
                  <tr key={p.id} className="hover:bg-[#FDFCFB]/50 transition-colors">
                    ...
                    <td className="p-4 text-xs">
                      <div className="font-semibold text-[#171513]">{p.usuarios?.nome_completo}</div>
                      <div className="text-[10px] text-gray-400 mt-0.5">{p.usuarios?.email}</div>
                    </td>
